...
function getVal(iDict, pos: number, offset: number, opCode: number, relativeBase: number, writable: boolean) {
    var rtn: number;
...
function rangeCheck(iDict, val: number) {
    if (!iDict[val]) {
        iDict[val] = 0;
    }
}

function calcOpCode3(opCodeFull: number): number[] {
